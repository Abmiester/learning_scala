...
object Huffman extends App {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def increment(acc: Map[Char, Int], char: Char) = {
      val count = (acc get char).getOrElse(0) + 1
      acc + ((char, count))
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def lookup(tree: CodeTree)(char: Char): List[Bit] = tree match {
        case Leaf(_, _) => List()
        case Fork(left, right, _, _) if chars(left).contains(char) => 0 :: lookup(left)(char)
        case Fork(left, right, _, _) => 1 :: lookup(right)(char)
      }
